...
def get_frame():
    ...
    folder = os.getcwd() + '/SiamMask/data/multi_frames/'
...
def movecam(frame):
    ...
    os.system("python SiamMask/tools/demo.py --config SiamMask/experiments/siammask_sharp/config_davis.json --resume SiamMask/experiments/siammask_sharp/SiamMask_DAVIS.pth --base_path SiamMask/data/multi_frames")
    orig_area = 60*85 # in pixels

    if frame < 3:
        
        # read in estimated target size and location
        nextloc = np.loadtxt('multi_modelview/nextloc%d.txt'%(frame+1))
        ...
        # take in previous states as well
        loc1 = np.loadtxt('multi_modelview/nextloc%d.txt'%(frame-2))
        loc2 = np.loadtxt('multi_modelview/nextloc%d.txt'%(frame-1))
        loc3 = np.loadtxt('multi_modelview/nextloc%d.txt'%(frame-0))
        loc4 = np.loadtxt('multi_modelview/nextloc%d.txt'%(frame+1))

        #average over the states
        new_area = 0.1*loc1[1, 0]*loc1[1, 1] + 0.2*loc2[1, 0]*loc2[1, 1] + 0.3*loc3[1, 0]*loc3[1, 1] + 0.4*loc4[1, 0] * loc4[1, 1]
        new_x = 0.1*loc1[0, 0] + 0.2*loc2[0, 0] + 0.3*loc3[0, 0] + 0.4*loc4[0, 0]
        new_y = 0.1*loc1[0, 1] + 0.2*loc2[0, 1] + 0.3*loc3[0, 1] + 0.4*loc4[0, 1] 
        x_offset = 960/2 - new_x
        z_offset = 540/2 - new_y
        y_offset = orig_area/new_area
