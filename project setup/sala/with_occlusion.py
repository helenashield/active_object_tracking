...
def get_frame():
    ...
    folder = os.getcwd() + '/SiamMask/data/occ_frames/'
...
def movecam(frame):
    ...
    os.system("python SiamMask/tools/demo.py --config SiamMask/experiments/siammask_sharp/config_davis.json --resume SiamMask/experiments/siammask_sharp/SiamMask_DAVIS.pth --base_path SiamMask/data/occ_frames")


    # get occlusion prediction
    os.system("python occ_prediction.py")

    preds = np.loadtxt('preds.txt')
    pred = np.argmax(preds)
    scale = [0.0, 0.075, 0.225, 0.375, 0.525, 0.675, 0.825, 0.95, 100]
    scaler = scale[9-pred]


    orig_area = 60*85 # in pixels
    # read in estimated target size and location
    nextloc = np.loadtxt('occ_modelview/nextloc%d.txt'%(frame+1))
    new_area = nextloc[1, 0]*nextloc[1, 1]
    x_offset = 960/2 - nextloc[0, 0]
    z_offset = 540/2 - nextloc[0, 1]
    y_offset =  orig_area/(new_area + 0.1/(S + 0.1))
